...
def evaluator(sparse_pred, sparse_gt, raw_gt, nmse_raw=False):
    ...
    with torch.no_grad():
        ...
        if nmse_raw:
            ...
            sparse_pred_mod = sparse_pred.permute(0, 2, 3, 1)  # Move the real/imaginary dim to the last
            zeros = sparse_pred_mod.new_zeros((n, nt, nc_expand - nc, 2))
            # When pytorch version is above 1.7.0, complex number representation is changed from [a, b] to [a, b.j]
            if version.parse(torch.__version__) > version.parse("1.7.0"):
                sparse_pred_mod = torch.view_as_complex(torch.cat((sparse_pred_mod, zeros), dim=2))
                raw_pred = torch.view_as_real(torch.fft.fft(sparse_pred_mod))[:, :, :125, :]
            else:
                sparse_pred_mod = torch.cat((sparse_pred, zeros), dim=2)
                raw_pred = torch.fft(sparse_pred_mod, signal_ndim=1)[:, :, :125, :]
            power_gt_raw = raw_gt[..., 0] ** 2 + raw_gt[..., 1] ** 2
            difference_raw = raw_gt - raw_pred
            mse_raw = difference_raw[..., 0] ** 2 + difference_raw[..., 1] ** 2
            nmse_raw = 10 * torch.log10((mse_raw.sum(dim=[1, 2]) / power_gt_raw.sum(dim=[1, 2])).mean())
            print(f'NMSE (raw): {nmse_raw}')
